...
export interface SocShellContextValue {
...
export const SocShellContext = React.createContext<SocShellContextValue | null>(null);

export interface SocShellProps {
  activeNav: NavItemKey;
  onNavigate: (route: string) => void;
  children: React.ReactNode;
}

export const SocShell: React.FC<SocShellProps> = ({
  activeNav,
  onNavigate,
  children,
}) => {
  ...
  // Ping backend health once globally for the shell
  useEffect(() => {
    let mounted = true;
  ...
  // When activeNav changes, reset scroll to top of workspace
  useEffect(() => {
  ...
  return (
    <SocShellContext.Provider value={{ isInsideShell: true, backendHealth }}>
      <div
        className="w-full h-screen flex text-foreground font-sans transition-colors duration-300 antialiased overflow-hidden bg-depth-ambient relative"
        style={{
          background:
            "radial-gradient(ellipse 80% 50% at 50% -20%, rgba(6, 182, 212, 0.06), transparent 70%), linear-gradient(135deg, #05070d 0%, #080d1a 50%, #050812 100%)",
        }}
      >
        ...
        {/* ─── MAIN WORKSPACE SCROLL COLUMN ───────────────────────────────────── */}
        <div
          ref={mainScrollRef}
          className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto overflow-x-hidden relative z-10 custom-scrollbar"
        >
          {children}
        </div>
      </div>
...
interface SocLayoutContentProps extends SocLayoutProps {
  backendHealth: {
    status: "healthy" | "offline" | "checking";
    version?: string;
  };
...
const SocLayoutContent: React.FC<SocLayoutContentProps> = ({
  onNavigate,
  title,
  subtitle,
  badge,
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 relative">
      {/* Top Header */}
      <header
        className="h-16 px-6 border-b border-cyan-500/15 flex items-center justify-between gap-4 sticky top-0 z-20 backdrop-blur-xl flex-shrink-0"
        style={{
      ...
      {(subtitle || actions) && (
        <div className="px-6 py-4 border-b border-white/5 flex flex-wrap items-center justify-between gap-4 bg-slate-900/20 backdrop-blur-sm flex-shrink-0">
          <div>
...
export const SocLayout: React.FC<SocLayoutProps> = (props) => {
  const shellCtx = React.useContext(SocShellContext);

  if (shellCtx?.isInsideShell) {
    return <SocLayoutContent {...props} backendHealth={shellCtx.backendHealth} />;
  }

  // Fallback if rendered standalone without SocShell
  return (
    <SocShell activeNav={props.activeNav} onNavigate={props.onNavigate}>
      <SocLayoutContent {...props} backendHealth={{ status: "checking" }} />
    </SocShell>
  );
};
